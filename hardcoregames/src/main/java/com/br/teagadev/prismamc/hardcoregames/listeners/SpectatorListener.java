...
public class SpectatorListener implements Listener {
   ...
   public void onChat(AsyncPlayerChatEvent event) {
      ...
      if (!VanishAPI.inAdmin(player) && !GamerManager.getGamer(player.getUniqueId()).isPlaying()) {
         event.setCancelled(true);
      }
   ...
   public void onPlayerDamage(PlayerDamagePlayerEvent event) {
      if (VanishAPI.inAdmin(event.getPlayer())) {
         event.setCancelled(true);
      } else if (!GamerManager.getGamer(event.getPlayer().getUniqueId()).isPlaying()) {
         event.setCancelled(true);
      } else if (VanishAPI.inAdmin(event.getDamaged())) {
         event.setCancelled(true);
      } else if (!GamerManager.getGamer(event.getDamaged().getUniqueId()).isPlaying()) {
         event.setCancelled(true);
      }
   }
   ...
   public void onPickup(PlayerPickupItemEvent event) {
      if (!GamerManager.getGamer(event.getPlayer().getUniqueId()).isPlaying()) {
         event.setCancelled(true);
      } else if (VanishAPI.inAdmin(event.getPlayer())) {
         event.setCancelled(true);
      }
   }
