...
import com.br.teagadev.prismamc.hardcoregames.menu.KitSelector;
...
public class PreGameListeners implements Listener {
   ...
   public void onTimer(GameTimerEvent event) {
      ...
      if (HardcoreGamesMain.getTimerManager().getLastAlive() < 10) {
         ...
         if (event.getTime() == 15) {
            Bukkit.getOnlinePlayers().forEach((onlines) -> {
               onlines.teleport(HardcoreGamesUtility.getRandomLocation(20));
            });
         }
   ...
   public void onStart(GameStageChangeEvent event) {
      if (event.getLastStage() == GameStages.WAITING && event.getNewStage() == GameStages.INVINCIBILITY) {
