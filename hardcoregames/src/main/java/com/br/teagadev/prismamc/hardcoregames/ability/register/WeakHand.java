...
public class WeakHand extends Kit {
   public WeakHand() {
   ...
   public void onPlayerDamage(PlayerDamagePlayerEvent event) {
      if (this.useAbility(event.getPlayer()) && Math.random() <= 0.15D) {
         event.getDamaged().addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS, 100, 1));
