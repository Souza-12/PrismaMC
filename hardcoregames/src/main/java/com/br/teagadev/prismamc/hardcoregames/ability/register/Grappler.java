...
public class Grappler extends Kit {
   private Map<Player, Grappler.Cordinha> hooks = new HashMap();
   ...
   public Grappler() {
      ...
      this.setItens(new ItemStack[]{(new ItemBuilder()).material(Material.LEASH).name(this.getItemColor() + "Kit " + this.getName()).build()});
   }
   ...
   public void onFall(EntityDamageEvent event) {
      if (event.getEntity() instanceof Player) {
         if (event.getCause() == DamageCause.FALL) {
            ...
            if (this.hooks.containsKey(player) && ((Grappler.Cordinha)this.hooks.get(player)).isHooked() && event.getDamage() > 5.0D) {
   ...
   public void onLeash(PlayerLeashEntityEvent e) {
      if (this.containsHability(e.getPlayer())) {
         e.setCancelled(true);
   ...
   public void usar(PlayerInteractEvent e) {
      ...
      if (p.getItemInHand().getType().equals(Material.LEASH) && this.containsHability(p)) {
         e.setCancelled(true);
         ...
         if (e.getAction() != Action.LEFT_CLICK_AIR && e.getAction() != Action.LEFT_CLICK_BLOCK) {
            ...
            nmsHook.move(p.getLocation().getDirection().getX() * 2.5D, p.getLocation().getDirection().getY() * 2.5D, p.getLocation().getDirection().getZ() * 2.5D);
   ...
   protected void clean(Player player) {
      ...
      if (this.leftClickGrappler.containsKey(player)) {
         this.leftClickGrappler.remove(player);
      }

      if (this.rightClickGrappler.containsKey(player)) {
